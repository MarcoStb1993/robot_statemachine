...
void StatemachineControlPanel::initCommunications() {
	...
	_operation_mode_subcriber =
			nh.subscribe < statemachine_msgs::OperationMode
					> ("operationMode", 1, &StatemachineControlPanel::operationModeCallback, this);
	...
	_reverse_mode_subscriber =
			nh.subscribe < std_msgs::Bool
					> ("reverseMode", 10, &StatemachineControlPanel::reverseModeCallback, this);
...
void StatemachineControlPanel::stateInfoCallback(
	...
	if (state_info->data.compare("Idle") == 0
		...
		if (!_exploration_running) {
			_gui->start_exploration_button->setText("Pause");
			_exploration_running = true;
			_gui->exploration_info_text->setText("Exploration running");
		}
	} else if (state_info->data.compare("Waypoint Following") == 0
			|| state_info->data.compare("Navigation: Waypoint Following")
					== 0) {
		if (!_waypoint_following_running) {
			_gui->start_waypoint_following_button->setText("Stop");
			_waypoint_following_running = false;
...
PLUGINLIB_EXPORT_CLASS(statemachine::StatemachineControlPanel, rqt_gui_cpp::Plugin)
